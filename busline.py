...
def init(ll, tt):
    ...
    driver.get('https://amap.com/service/poiInfo?query_type=TQUERY&city=320500&keywords=7313路')
...
def getbusline(linename, datajson):
    ...
    try:
        ...
        id = datajson['id']
        code = datajson['code']
        front_name = datajson['front_name']
        terminal_name = datajson['terminal_name']
...
poi_bus_url = "https://amap.com/service/poiInfo?query_type=TQUERY&city=320500&keywords="
read_file_dir = "公交线路06021713.xls"  # 公交线路名
ll = 1
tt = 0
nn = 1
nnn = 1
...
busname = "公交线路坐标" + time.strftime("%m%d%H%M", time.localtime())
busstation = "公交站点坐标" + time.strftime("%m%d%H%M", time.localtime())
# 获取列数
